...
function Rating({ rating, maxRating = 5, className = '' }) {  // Paramètres par défaut pour maxRating et className
  // Assurez-vous que rating est un nombre valide
  ...
  return (
    <div className={`rating ${className}`}>
      {[...Array(maxRating)].map((_, index) => (
        <span
          key={index}
          className={`${className}__star ${index < validRating ? `${className}__star--filled` : ''}`}
        >
...
Rating.propTypes = {
  rating: PropTypes.number.isRequired,  // `rating` doit être un nombre et est requis
  maxRating: PropTypes.number,          // `maxRating` doit être un nombre et est optionnel
  className: PropTypes.string,          // `className` doit être une chaîne de caractères et est optionnel
};
